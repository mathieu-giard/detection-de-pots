...
public class MainClass {
	...
	public boolean comparaisonDescripteursCarre(Complex[] test, float seuil) {
		...
		double diff = 0;
		for (int i = 2; i < Math.min(test.length, refCarre.length); i++) { // commence é deux pour ne pas prendre en compte la taille du carre
	...
	public boolean IsCarre( Point[] signature, double seuil){
		...
		for (int k=0; k<72; k++){
			...
			Ck=Ck/(sigma1*sigma2);
			//if (Ck<0){ Ck = - Ck ;} //je suis pas sér qu'il faille prendre la val abs mais 
			C= Math.max(Ck, C);	
		}
